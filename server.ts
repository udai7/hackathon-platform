...
const getAllHackathons = (req: Request, res: Response): void => {
  if (isUsingFallback) {
    ...
    console.log(
      `Returning ${inMemoryHackathons.length} hackathons from memory`
    );
  ...
  HackathonModel.find()
    ...
    .catch((error) => {
      ...
      // Fallback to in-memory data
      console.log(
        `Falling back to in-memory: returning ${inMemoryHackathons.length} hackathons`
      );
      res.status(200).json(inMemoryHackathons);
    });
};
...
const getHackathonsByCreator = (req: Request, res: Response): void => {
  ...
  if (isUsingFallback) {
    console.log(
      `Using in-memory data for GET /api/hackathons/creator/${creatorId}`
    );
    const userHackathons = inMemoryHackathons.filter(
      (h) => h.creatorId === creatorId
    );
    console.log(
      `Found ${userHackathons.length} hackathons for creator ${creatorId} in memory`
    );
  ...
  HackathonModel.find({ creatorId })
    ...
    .then((hackathons) => {
      console.log(
        `Found ${hackathons.length} hackathons for creator ${creatorId} in database`
      );
    ...
    .catch((error) => {
      ...
      console.log(
        `Falling back to in-memory: found ${userHackathons.length} hackathons for creator ${creatorId}`
      );
      res.status(200).json(userHackathons);
    });
};
...
const createHackathon = (req: Request, res: Response): void => {
  ...
  if (hackathonData.organizerName === "")
    hackathonData.organizerName = "Organizer";
  if (hackathonData.location === "") hackathonData.location = "Online";
  if (hackathonData.image === "")
    hackathonData.image = "https://via.placeholder.com/800x400?text=Hackathon";
  if (hackathonData.prizes === "") hackathonData.prizes = "To be announced";
  if (hackathonData.teamSize === "") hackathonData.teamSize = "1-5";
  if (hackathonData.registrationFee === "")
    hackathonData.registrationFee = "Free";
  if (hackathonData.website === "") hackathonData.website = "#";

  // Check if image is a data URL and it's too large (>1MB)
  if (
    hackathonData.image &&
    hackathonData.image.startsWith("data:image/") &&
    hackathonData.image.length > 1000000
  ) {
    console.warn(
      "Image data URL is too large:",
      (Number(hackathonData.image.length) / 1000000).toFixed(2) + "MB"
    );
    // Limit to a reasonable size to prevent MongoDB document size limits
    res.status(400).json({
      message:
        "Image data is too large. Please use a smaller image or provide a URL instead.",
  ...
  newHackathon
    ...
    .then((savedHackathon) => {
      console.log(
        "Successfully saved hackathon to database:",
        savedHackathon.id
      );
    ...
    .catch((error) => {
      ...
      if (
        error.message &&
        error.message.includes("document is larger than the maximum")
      ) {
        res.status(400).json({
          message:
            "Hackathon data is too large. Please reduce the size of your image or use an external image URL.",
...
const updateHackathon = (req: Request, res: Response): void => {
  ...
  HackathonModel.findOneAndUpdate({ id }, hackathonData, { new: true })
...
const registerParticipant = (req: Request, res: Response): void => {
  ...
  if (isUsingFallback) {
    console.log(
      `Using in-memory data for POST /api/hackathons/${id}/participants`
    );
    const index = inMemoryHackathons.findIndex((h) => h.id === id);
    if (index === -1) {
      res.status(404).json({ message: "Hackathon not found" });
    ...
    if (existingParticipant) {
      res
        .status(400)
        .json({ message: "You have already registered for this hackathon" });
      return;
    }

    // Check if payment is required
    const paymentRequired = inMemoryHackathons[index].paymentRequired || false;
    participantData.paymentStatus = paymentRequired
      ? "pending"
      : "not_required";
  ...
  HackathonModel.findOne({ id })
    ...
    .catch((error) => {
      ...
      const paymentRequired =
        inMemoryHackathons[index].paymentRequired || false;
      participantData.paymentStatus = paymentRequired
        ? "pending"
        : "not_required";
...
const withdrawParticipant = (req: Request, res: Response): void => {
  ...
  if (isUsingFallback) {
    console.log(
      `Using in-memory data for DELETE /api/hackathons/${hackathonId}/participants/${participantId}`
    );
    ...
    const participantIndex = inMemoryHackathons[index].participants.findIndex(
...
const createPaymentOrder = (req: Request, res: Response): void => {
  ...
  if (isUsingFallback) {
    res
      .status(503)
      .json({ message: "Payment processing not available in fallback mode" });
    return;
  }
  ...
  HackathonModel.findOne({ id: hackathonId })
    .then((hackathon) => {
      ...
      return razorpayService
        .createOrder(Number(amount), receiptId)
        .then((order) => {
          // Save payment record
    ...
    .then((result) => {
      res.status(201).json({
        ...
        order: {
          id: result.order.id,
          amount: Number(result.order.amount) / 100, // Convert back to rupees for client
    ...
    .catch((error) => {
      ...
      if (error instanceof Error) {
        if (error.message === "Hackathon not found") {
          ...
          error.message === "Payment not required for this hackathon"
        ) {
          res
            .status(400)
            .json({ message: "Payment not required for this hackathon" });
          return;
        }
      }
...
const verifyPayment = (req: Request, res: Response): void => {
  ...
  if (isUsingFallback) {
    res
      .status(503)
      .json({ message: "Payment verification not available in fallback mode" });
    return;
  }
  ...
  PaymentModel.findOne({ orderId })
    ...
    .then((updatedPayment) => {
      ...
      return HackathonModel.findOne({ id: updatedPayment.hackathonId }).then(
        (hackathon) => {
          if (!hackathon) {
            throw new Error("Hackathon not found");
          }
          ...
          const participantIndex = hackathon.participants?.findIndex(
            (p) => p.id === updatedPayment.participantId
          );
          ...
          if (hackathon.participants) {
            hackathon.participants[participantIndex].paymentStatus =
              "completed";
...
const updateHackathonPaymentDetails = (req: Request, res: Response): void => {
  ...
  HackathonModel.findOne({ id })
    ...
    .catch((error) => {
      ...
      res
        .status(500)
        .json({ message: "Failed to update hackathon payment details" });
    });
};

// Submit project
const submitProject = async (req: Request, res: Response): Promise<void> => {
  // support sending hackathonId either in params or in the body
  const hackathonId = req.params.hackathonId || req.body.hackathonId;
  const { participantId, githubLink, projectDescription, liveUrl } = req.body;
  ...
  try {
    ...
    // Persist liveUrl when provided and keep existing evaluation/ranking fields
...
const evaluateProject = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Project evaluated successfully", evaluation });
  } catch (error) {
...
const rankHackathonProjects = async (
  req: Request,
  res: Response
): Promise<void> => {
    ...
    rankings.forEach((ranking) => {
      const participant = (hackathon.participants || []).find(
        (p) =>
          ...
          submittedProjects[ranking.index].githubLink
      );
      if (participant && participant.projectSubmission) {
  ...
  try {
    ...
    (hackathon.participants || []).forEach((participant) => {
      if (participant.university) {
        analytics.universities[participant.university] =
          (analytics.universities[participant.university] || 0) + 1;
      }
    });
    ...
    (hackathon.participants || []).forEach((participant) => {
      if (participant.projectSubmission) {
        ...
        if (participant.projectSubmission.evaluation) {
          ...
          analytics.submissionStats.averageScore +=
            participant.projectSubmission.evaluation.score;
        }
      }
    });
...
app.listen(PORT, () => {
  ...
  if (isUsingFallback) {
    ...
    console.warn(
      "To use MongoDB, please check your connection settings in the .env file\n"
    );
  }
});
